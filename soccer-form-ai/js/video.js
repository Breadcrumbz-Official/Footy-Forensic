...
const MAX_CLIP_FRAMES = 60; // safety cap if a user drags a very wide manual range

/**
 * Sample every video frame between `start` and `end` (inclusive), spaced one
 * assumed frame apart at `fps`. Shared by the automatic and manual clip
 * pickers below. If the range would produce more than MAX_CLIP_FRAMES,
 * subsamples evenly rather than seeking hundreds of times.
 */
async function sampleRange(videoEl, start, end, fps, onProgress) {
  const dt = 1 / Math.max(1, fps);
  const dur = isFinite(videoEl.duration) ? videoEl.duration : Math.max(start, end);
  const lo = Math.max(0, Math.min(start, end));
  const hi = Math.min(Math.max(0, dur - 0.001), Math.max(start, end));

  let times = [];
  for (let t = lo; t <= hi + 1e-6; t += dt) times.push(t);
  if (times.length === 0) times.push(lo);
  if (times.length > MAX_CLIP_FRAMES) {
    const step = (times.length - 1) / (MAX_CLIP_FRAMES - 1);
    times = Array.from({ length: MAX_CLIP_FRAMES }, (_, i) => times[Math.round(i * step)]);
  }

  const frames = [];
  for (const t of times) {
    await seekTo(videoEl, t);
    frames.push({ time: videoEl.currentTime, canvas: captureFrame(videoEl) });
    onProgress?.(frames.length, times.length);
  }
  return frames;
}

function nearestIdx(frames, t) {
...
/**
 * Capture a short burst of frames straddling `centerTime`, spaced one video
 * frame apart. A single picked instant is often the most motion-blurred one
 * in the whole kick (the contact frame especially — the foot is moving
 * fastest right there), which throws off pose landmarks. Handing the pose
 * model a few frames of real motion either side, in order, lets it use
 * temporal context instead of judging that one blurry instant alone.
 *
 * Returns the clip plus the index of the frame nearest the requested time —
 * callers use that as the frame to score, falling back to a neighbour only if
 * no person was found exactly there.
 */
export async function captureWindow(videoEl, centerTime, { spanMs = 260, fps = 30, onProgress } = {}) {
  const half = spanMs / 2000;
  const frames = await sampleRange(videoEl, centerTime - half, centerTime + half, fps, onProgress);
  const centerIdx = nearestIdx(frames, centerTime);
  await seekTo(videoEl, centerTime); // leave the player showing what the user picked
  return { frames, centerIdx };
}

/**
 * Same idea as captureWindow, but with the clip boundaries chosen by the user
 * (a manual "clip start" / "clip end" pick) instead of an automatic ±span.
 * `centerTime` is still the exact instant being scored; it does not need to
 * be the midpoint of [start, end].
 */
export async function captureRange(videoEl, start, end, centerTime, fps = 30, onProgress) {
  const frames = await sampleRange(videoEl, start, end, fps, onProgress);
  const centerIdx = nearestIdx(frames, centerTime);
  await seekTo(videoEl, centerTime);
  return { frames, centerIdx };
}
